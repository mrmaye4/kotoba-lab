...
import { ruleCategories } from '@/lib/db/schema'
...
export async function GET(request: NextRequest) {
  ...
  const rows = await db
    .select()
    .from(ruleCategories)
    .where(and(eq(ruleCategories.languageId, languageId), eq(ruleCategories.userId, user.id)))
...
export async function DELETE(request: NextRequest) {
  ...
  await db
    ...
    .where(and(eq(ruleCategories.id, id), eq(ruleCategories.userId, user.id)))
